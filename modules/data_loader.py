...
from .dataset import (
    StitchingNet_make_all_samples, StitchingNetVer2_make_all_samples, 
    ImageNet_1k_make_all_samples, ImageNet_tiny_make_all_samples, 
    stratified_split, DefectDataset, 
    get_train_transform_v1, get_train_transform_v2, get_train_transform_v3, get_val_test_transform
)
...
def get_defect_data_loaders(
    ...
    print(f"[INFO] Loading dataset: {dataset_name} from {data_root}")
    print(f"[INFO] image size: {img_size}, batch size: {batch_size}, num_workers: {num_workers}")

    # 1) 전체 samples와 classes 생성
    if dataset_name.startswith("StitchingNet"):
    ...
    # ImageNet은 test 데이터셋의 정답 레이블이 없으므로 val_samples를 test_samples로 사용    
    elif dataset_name == "ImageNet-1k":
        train_samples, classes = ImageNet_1k_make_all_samples(data_root, split="train" )
        val_samples, _ = ImageNet_1k_make_all_samples(data_root, split="val")
        test_samples = val_samples  # ImageNet-1k는 test 데이터셋의 정답 레이블이 없음
    elif dataset_name == "ImageNet-tiny":
        train_samples, classes = ImageNet_tiny_make_all_samples(data_root, split="train" )
        val_samples, _ = ImageNet_tiny_make_all_samples(data_root, split="val")
        test_samples = val_samples
    elif dataset_name == "ImageNet-21k":
        raise NotImplementedError("ImageNet-21k dataset loading is not implemented yet.")
    ...
    if split_output_dir and not  dataset_name.startswith("ImageNet"):
    ...
    # 4) transform 정의
    transform_map = {
        1: (get_train_transform_v1(img_size), "version 1"),
        2: (get_train_transform_v2(img_size), "version 2"),
        3: (get_train_transform_v3(img_size), "version 3"),
        0: (get_val_test_transform(img_size), "version 0"),
    }

    train_transform, version_name = transform_map.get(use_augmentation, transform_map[0])
    print(version_name)

    val_test_transform = get_val_test_transform(img_size)
