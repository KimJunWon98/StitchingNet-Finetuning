...
class EarlyStopping:
    """
    - patience: 개선 없는 epoch가 'patience'에 도달 시 조기 종료
    - model_name: 체크포인트 파일명에 사용할 모델 이름 (예: 'resnet50')
    - layers: freeze/unfreeze 레이어 정보를 문자열로 합친 것 (파일명에 표시)
    - 저장 기준: val_loss가 최소값을 갱신할 때만 best 모델로 저장
    """
    ...
    def save_checkpoint(self, val_loss, model, checkpoint_dir):
        """
        val_loss가 개선될 때만 호출되어 Best checkpoint를 저장
        """
...
def train_model(
    ...
    """
    - EarlyStopping으로 BEST 모델(최소 val_loss)을 저장
    - 매 epoch마다 checkpoint_dir에 epoch별 체크포인트를 저장하되,
      최근 max_checkpoints개만 보관 (기본적으로 patience 개)
    - layers: freeze/unfreeze 식별용 문자열
    - model_name: 모델 이름
    """
    import wandb
    from collections import deque
    ...
    for epoch in range(num_epochs):
        ...
        # 3-1) Deque에 추가 → 초과 시 가장 오래된 ckpt 제거
